...
class MeanAp:
    ...
    def calc_mean_ap(self, label_file_name, PR_curve):
        ap = []
        ...
        for cls in range(num_cls):
            ...
            ap.append(self.calc_ap(detected, num_truth, cls))


        return sum(ap)/num_cls, ap

    def calc_ap(self, detected, num_truth, cls):
        k = lambda x: x['conf']
        ...
        if self.PR_curve:
            ...
            plt.title('Precision-Recall Curve 2D {}'.format(self.label_file_name[cls]))
            if not os.path.isdir("./PR"):
                os.makedirs("./PR")
            plt.savefig('./PR/{}.png'.format(self.label_file_name[cls]))
